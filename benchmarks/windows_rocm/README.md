...
On the 40k x 3000 H64 production shape, `pipeline_fancy` split extraction copies about 457.8 MiB before Dataset creation. With pristine v4.7 this path is about 3.0 s for Dataset construction and peaks about 2.0 GiB above the resident phase matrix. The fork's dense row-major float32 builder now constructs the same serialized Dataset in about 1.02-1.06 s; even with the unchanged pipeline copy the peak drops to about 600 MiB above the phase matrix. With `contiguous_slice`, the split shares the phase buffer and the optimized Dataset path peaks only about 140 MiB above it. H64/H128 production binaries produced by the optimized path are SHA-256 identical to pristine v4.7, and dedicated dense-zero/NaN, sparse-EFB, categorical, and reference-Dataset probes also produced byte-identical binaries.
...
python .\benchmarks\windows_rocm\run_dataset_benchmarks.py --stage end_to_end --profile h64 --iterations 100 --split-extraction contiguous_slice
```
