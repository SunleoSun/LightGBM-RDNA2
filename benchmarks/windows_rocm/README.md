...
On the 40k x 3000 H64 production shape, current `pipeline_fancy` split extraction copied about 457.8 MiB and took about 92 ms before Dataset creation; the total process peak rose about 2.01 GiB above the already-resident phase matrix. `contiguous_slice` took effectively zero time, shared the phase buffer, added no split working set, reduced the peak to about 1.55 GiB, and produced an identical pristine-v4.7 CPU model/prediction result.
