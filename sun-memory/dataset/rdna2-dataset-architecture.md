...
Histogram readback now uses reusable pinned host staging memory on a persistent HIP stream. The device histogram is copied asynchronously into pinned staging, the stream is synchronized once, and the result is copied into the Serial-owned canonical histogram buffer. This keeps ownership explicit while avoiding device-wide synchronization.
