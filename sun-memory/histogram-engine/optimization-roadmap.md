...
The immediate optimization order is now: (1) strict-exact temporal aggregation experiments that reduce double LDS atomics only if the CPU-exact gate survives; (2) root/large-leaf specialization and H64/H128 workgroup geometry; (3) only then wave32 aggregation experiments. Constant-Hessian scalar specialization was tested separately and rejected because it reduced Hessian H2D slightly but made the H128 regression kernel slower. Moving objective, best-split, or float accumulation onto GPU remains outside the strict-exact production path.
