...
The immediate optimization order is now: (1) move from repeated host-to-device leaf-index copies toward a minimal RDNA2 device mirror of Serial `DataPartition` state while keeping Serial as the canonical owner; (2) reduce full-grid histogram memset/D2H for scattered active features through compact active tuple/range descriptors; (3) then revisit root/constant-Hessian specialization and low-level H64/H128 kernel tuning. Moving objective or best-split wholesale to GPU remains later because their current costs do not justify the semantic risk.
