...
Phase 1 foundation is now implemented: `RDNA2HistogramEngine` is owned by `RDNA2TreeLearner`, builds exact canonical `uint32 feature4[group][row]` packed storage once, uploads it to persistent gfx1030 device memory, and leaves `ConstructHistograms` on the Serial correctness fallback. The full smoke suite remains prediction-diff `0` with exact CPU tree structure. The immediate next engineering task is the first dense H64 HIP histogram producer consuming this packed input; enable it only after its output can satisfy the same CPU 4.7 correctness gate.
