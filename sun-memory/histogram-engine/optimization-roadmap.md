...
The immediate optimization order is therefore: (1) remove the staging-to-canonical histogram memcpy by making the canonical RDNA2 histogram destination pinned/registered host memory; (2) honor and compact the canonical dynamic `is_feature_used` mask for every tree instead of calculating all features. This must never assume `feature_fraction=0.9`, a fixed active count, or a contiguous feature range: `ColSampler` randomly samples valid feature indices using `feature_fraction_seed`, and configuration can range from `(0,1]`; (3) pipeline GPU histogram chunks and D2H with CPU `FixHistogram`/subtraction/best-split scanning for the previous chunk; (4) experimentally split only the currently active features between CPU and RDNA2 so CPU constructs a measured minority concurrently while GPU handles the rest; (5) only then consider GPU-resident leaf partitioning to eliminate repeated index H2D. Moving best-split wholesale to GPU remains later because CPU split logic is already parallel and exact, and OpenCL also keeps it on CPU. The `feature_fraction` benchmark suite at 0.5/0.75/0.9/1.0 is mandatory for stages 2-4.
