...
The immediate optimization order is therefore: (1) remove the staging-to-canonical histogram memcpy by making the canonical RDNA2 histogram destination pinned/registered host memory; (2) exploit the H64 feature mask because only ~2700/3000 features are active on average, whereas H128 uses ~2974/3000; (3) pipeline GPU histogram chunks and D2H with CPU `FixHistogram`/subtraction/best-split scanning for the previous chunk; (4) experimentally split dense feature ownership between CPU and RDNA2 so CPU constructs a measured minority of features concurrently while GPU handles the rest; (5) only then consider GPU-resident leaf partitioning to eliminate repeated index H2D. Moving best-split wholesale to GPU remains later because CPU split logic is already parallel and exact, and OpenCL also keeps it on CPU.
