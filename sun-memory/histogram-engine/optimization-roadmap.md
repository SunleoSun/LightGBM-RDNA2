...
The immediate optimization order is now: (1) pipeline GPU histogram chunks and direct D2H with CPU `FixHistogram`/subtraction/best-split scanning for the previous chunk; (2) experimentally split only the currently active features between CPU and RDNA2 so CPU constructs a measured minority concurrently while GPU handles the rest; (3) only then consider GPU-resident leaf partitioning to eliminate repeated index H2D. Moving best-split wholesale to GPU remains later because CPU split logic is already parallel and exact, and OpenCL also keeps it on CPU. The `feature_fraction` benchmark suite remains mandatory for chunking/ownership changes.
