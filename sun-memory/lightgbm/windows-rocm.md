...
A strict matrix covers H64/H128 baselines, no-bagging variants, feature_fraction=0.5, strong regularization, scale_pos_weight=16, and regression_l2. The fast smoke gate uses h64, h128, h64_scale16, and h128_regression at 6 trees, runs only LightGBM 4.7 CPU plus ROCm, keeps the full 40,000 x 3,000 training shape, and uses 5,000 validation rows by default to reduce prediction I/O. Stress covers all twelve profiles at 20 trees with all four backends; production uses H64/H128 at 100 trees with all four backends. The repository checkout at C:\Drive\MyPrograms\LightGBM-RDNA2 is on Google Drive and must remain source-only. All generated work lives under C:\Temp\LightGBM-RDNA2 by default: build\cpu and build\rocm for CMake/Ninja builds, benches\bin for benchmark executables/DLLs, benches\data for generated datasets, and benches\artifacts for models, predictions, logs, and matrix summaries. Historical benchmark datasets/logs/models from the previous C:\Temp\lightgbm-rdna2-temp workspace were consolidated under C:\Temp\LightGBM-RDNA2\benches\legacy-pre-repo. The optimized smoke suite has a steady-state wall time around 14 seconds on the current machine once its validation cache exists; the first run can be slower while generating the 5,000-row validation cache. On the current gfx1030 feature4 implementation all twelve production-sized stress profiles fail against v4.7 CPU, including regression_l2, so the defect is not specific to binary probability conversion or class weighting. H128 with scale_pos_weight=16 can degenerate to a one-tree ROCm model. This makes restoration of H64/H128 correctness the first requirement of the new histogram architecture.
