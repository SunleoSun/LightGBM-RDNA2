...
The old OpenCL backend is also a performance reference. On production-shaped workloads historical measurements are roughly 18 ms/tree for H64 and 38 ms/tree for H128, while old H256 synthetic numbers around 5 ms/tree are not representative of current production targets. The first exact RDNA2 HIP references measured about `31.804 ms/tree` H64 and `45.525 ms/tree` H128. Stream-scoped async transfers plus pinned histogram readback subsequently improved H64 to about `30.7 ms/tree` in a representative 100-tree run while H128 remained approximately neutral around `46 ms/tree`; all runs retained prediction diff `0` and exact structure. Treat small differences between repeated CPU/RDNA2 timings as run-to-run variation rather than semantic regressions. Short six-tree RDNA2 timings are dominated by one-time ROCm initialization and should not be compared directly with amortized per-tree targets.
