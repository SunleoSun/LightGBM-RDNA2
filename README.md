...
The initial target is **Radeon RX 6800 XT / Navi21 / gfx1030** on Windows 11 with AMD ROCm/HIP SDK 6.2. The current development base is LightGBM 4.7.0. This repository keeps the upstream LightGBM history and license; project-specific work is developed on the `rocm-gfx1030-optimizations` branch.
...
The goal is to build a dedicated RDNA2 histogram engine for the wide, moderate-row workloads where the existing generic CUDA/HIP histogram architecture leaves substantial performance on the table. The main workload is approximately 40,000 training rows by 3,000 features and primarily uses `max_bin=63` or `max_bin=127`.
...
The generic LightGBM CPU/CUDA paths remain the fallback. RDNA2-specific kernels are intended to be explicit specializations rather than a collection of hidden changes to generic GPU behavior.
...
When configuring remotes, the intended layout is:
